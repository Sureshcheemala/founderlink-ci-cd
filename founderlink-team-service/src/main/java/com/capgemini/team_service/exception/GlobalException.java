...
public class GlobalException {
    ...
    public ResponseEntity<ErrorResponses> handleAccessDenied(
            AccessDeniedException ex,
            HttpServletRequest request) {
        return buildResponse("Access denied: You do not have permission to access this resource", HttpStatus.FORBIDDEN, request);
    }

    // 🔹 FEIGN (Microservice Communication)
    @ExceptionHandler(FeignException.class)
    public ResponseEntity<ErrorResponses> handleFeign(
            FeignException ex,
            HttpServletRequest request) {
        HttpStatus status = HttpStatus.resolve(ex.status());
        if (status == null) status = HttpStatus.INTERNAL_SERVER_ERROR;
        return buildResponse("External service error: " + ex.getMessage(), status, request);
    }
