...
import { FiArrowLeft, FiPlus, FiCpu, FiTarget, FiZap, FiGlobe } from "react-icons/fi";
...
const CreateStartup = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (!form.name || !form.description || !form.industry) {
      setError("Name, executive summary, and industry are required.");
      return;
    }
    try {
      setLoading(true);
      await createStartup({ ...form, fundingGoal: Number(form.fundingGoal) || 0 });
      setSuccess("Venture successfully published to the marketplace!");
      setTimeout(() => navigate("/my-startups"), 1200);
    } catch { setError("Failed to initialize venture listing."); }
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12 transition-colors duration-300">
      ...
      <div className="bg-white dark:bg-[#0f1513] border border-slate-200 dark:border-white/5 rounded-xl p-10 shadow-sm transition-colors duration-300">
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight mb-2 transition-colors duration-300">Launch New Venture</h1>
        <p className="text-slate-500 dark:text-slate-400 font-medium mb-10 transition-colors duration-300">Submit your startup profile to the FounderLink community.</p>
        ...
        <div className="space-y-12">
          ...
          <section>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="md:col-span-2">
                ...
                <input name="name" value={form.name} onChange={handleChange} className="input" placeholder="e.g. Nexus Dynamics" />
              </div>
              <div className="md:col-span-2">
                <Label>Tagline</Label>
                <input name="tagline" value={form.tagline} onChange={handleChange} className="input" placeholder="e.g. Revolutionizing decentralized finance." />
              </div>
              <div className="md:col-span-2">
                <Label required>Executive Summary</Label>
                <textarea name="description" value={form.description} onChange={handleChange} rows="4" className="input" placeholder="Summarize your mission and vision..." />
              </div>
              <div>
                <Label required>Industry Vertical</Label>
                <input name="industry" value={form.industry} onChange={handleChange} className="input" placeholder="e.g. AI / Fintech" />
              </div>
              <div>
          ...
          <section>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <input name="websiteUrl" value={form.websiteUrl} onChange={handleChange} className="input" placeholder="https://nexus.ai" />
              </div>
              <div>
                <Label>LinkedIn URL</Label>
                <input name="linkedinUrl" value={form.linkedinUrl} onChange={handleChange} className="input" placeholder="https://linkedin.com/company/nexus" />
              </div>
              <div className="md:col-span-2">
                <Label>Logo URL</Label>
                <input name="logoUrl" value={form.logoUrl} onChange={handleChange} className="input" placeholder="https://nexus.ai/logo.png" />
              </div>
            </div>
          </section>
          ...
          <section>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                <Label>The Market Gap (Problem)</Label>
                <textarea name="problemStatement" value={form.problemStatement} onChange={handleChange} rows="5" className="input" placeholder="Define the core market friction..." />
              </div>
              <div>
                <Label>The Architecture (Solution)</Label>
                <textarea name="solution" value={form.solution} onChange={handleChange} rows="5" className="input" placeholder="Describe your product architecture..." />
              </div>
            </div>
          </section>
          ...
          <section>
            ...
            <div className="max-w-sm">
              ...
              <input name="fundingGoal" type="number" value={form.fundingGoal} onChange={handleChange} className="input font-bold text-lg" placeholder="e.g. 5000000" />
            </div>
          </section>

          <div className="pt-8 border-t border-slate-100 dark:border-white/5 flex gap-5 transition-colors duration-300">
            <button onClick={handleSubmit} disabled={loading} className="btn-primary flex-1 py-4 text-base flex items-center justify-center gap-3">
              {loading ? (
                <span className="flex items-center gap-2">
                  <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                  Publishing...
                </span>
              ) : (<>Publish Venture <FiPlus /></>)}
            </button>
