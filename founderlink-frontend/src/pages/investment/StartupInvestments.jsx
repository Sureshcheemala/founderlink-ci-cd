    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="flex justify-between items-end mb-12 border-b border-slate-200 pb-8">
        <div>
          ...
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Investment Requests</h1>
          <p className="text-slate-600 mt-2 font-medium">Manage institutional capital commitments for your venture.</p>
        </div>
      </div>
      ...
      <div className="space-y-6">
        {investments.length === 0 && !loading ? (
          ...
          investments.map((inv) => (
            <div key={inv.id} className="card flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
              <div className="flex items-center gap-6">
                <div className="w-12 h-12 rounded-lg bg-slate-100 flex items-center justify-center text-slate-700 border border-slate-200">
                  <FiMail className="text-xl" />
                </div>
                <div>
                  <h3 className="text-lg font-bold text-slate-900">{inv.investorEmail}</h3>
                  <p className="text-xs text-slate-500 font-bold uppercase tracking-wider mt-1">Institutional Investor</p>
                </div>
              </div>

              <div className="bg-slate-50 px-8 py-4 rounded-xl border border-slate-100 min-w-[200px]">
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-1">Committed Amount</p>
                <p className="text-2xl font-bold text-slate-900">₹{inv.amount.toLocaleString()}</p>
              </div>
                ...
                {inv.status === "PENDING" ? (
                  <div className="flex gap-4 w-full">
                    ...
                    <button
                      ...
                      className="btn-secondary py-2.5 px-6 text-sm text-red-600 hover:text-red-700 hover:bg-red-50 flex-1 md:flex-none flex items-center justify-center gap-2"
                    >
                      <FiX /> Decline
                    </button>
                  </div>
